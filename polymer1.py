...
# Import class definitions from polymerClasses.py
class Position():
    """
...
def calculate_metrics(polymer):
    """
    Calculate metrics for a polymer molecule.
    """
    center_of_mass = polymer.centerOfMass.getTup()
    end_to_end_distance = polymer.endToEndDistance * 1e6  # Convert to micrometers
    radius_of_gyration = polymer.radiusOfGyration * 1e6  # Convert to micrometers
    return center_of_mass, end_to_end_distance, radius_of_gyration
...
def main():
    ...
    # Metrics storage
    center_of_mass_list = []
    end_to_end_distance_list = []
    radius_of_gyration_list = []

    # Simulation and metric calculation
    for _ in range(num_molecules):
        # Generate degree of polymerization from a normal distribution
        ...
        center_of_mass, end_to_end_distance, radius_of_gyration = calculate_metrics(polymer)
        center_of_mass_list.append(center_of_mass)
        end_to_end_distance_list.append(end_to_end_distance)
        radius_of_gyration_list.append(radius_of_gyration)

    # Calculate metrics averages and standard deviations
    avg_center_of_mass = np.mean(center_of_mass_list, axis=0)
    avg_end_to_end_distance = np.mean(end_to_end_distance_list)
    std_dev_end_to_end_distance = np.std(end_to_end_distance_list)
    avg_radius_of_gyration = np.mean(radius_of_gyration_list)
    std_dev_radius_of_gyration = np.std(radius_of_gyration_list)

    # Polydispersity index (PDI)
    pdi = std_dev_radius_of_gyration / avg_radius_of_gyration
